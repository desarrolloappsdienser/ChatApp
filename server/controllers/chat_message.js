...
function sendImage(req, res){
    console.log("Entra por sendImage");
...
async function getAll(req, res){
    console.log("CHAT GETALL",req.params);
...
async function getLastMessage(req, res){
    ...
    try {
        ...
        res.status(200).send(response || {});//aki decimos que si response tiene sms lo mandamos sino mandamos vacio
    } catch (error) {
        res.status(500).send({msg:"Error del servidor"});
    }
