...
function create(req,res){
    ...
    if(req.files.image){//no funciona...
    ...
    group.save((error,groupStorage)=>{
        if(error){
            ...
            if(!groupStorage){
                res.status(400).send({msg:"Error al crear grupo"});
            }else{
...
function getAll(req,res){
    ...
    Group.find({participants:user_id})
        ...
        .exec(async(error,groups)=>{
            console.log("PASA",groups);
            if(error){
                res.status(500).send({msg:"Error al obtener los grupos"});  
            }
            const arrayGroups = [];
            for await (const group of groups){
                console.log("groupXX",group._id);
                const response = await GroupMessage.findOne({group:group}).sort({
                    createdAt:-1,
                });
            ...
            console.log("GRUPOS",arrayGroups);
...
async function banearParticipant(req,res){
    ...
    res.status(200).send({msg:"Participante baneado con exito"}); 
}
